...
app.add_middleware(
    ...
    allow_methods=["*"],
    allow_headers=["*"],  # Inclui X-API-Key
)
...
async def api_key_middleware(request: Request, call_next):
    """
    Verifica API Key em todas as rotas protegidas.
    Roda DENTRO do CORSMiddleware — garante que respostas 401
    também recebem headers CORS (evita bloqueio no browser).
    """
    if API_KEY and request.method != "OPTIONS" and request.url.path not in _PUBLIC_PATHS:
