...
Endpoints:
    ...
    GET /btc                 — Empréstimo de ativos (BTC) B3
"""
...
from app.btc_data import fetch_btc_data, get_btc_for_ticker, debug_btc_fetch
...
_PUBLIC_PATHS = {"/", "/status", "/docs", "/openapi.json", "/redoc", "/btc/debug"}
...
async def get_screener(
    ...
    # ── Get ALL companies from CVM data ──
    empresas_df = data_service.get_companies()
    if empresas_df.empty:
        return {"data": [], "total": 0, "errors": 0, "metrics": [], "setores": []}

    logger.info(f"Screener: computando indicadores para {len(empresas_df)} empresas CVM")
    ...
    for _, emp in empresas_df.iterrows():
        try:
            cd_cvm = str(emp.get("CD_CVM", "")).strip()
            cnpj = str(emp.get("CNPJ_CIA", "")).strip()
            nome = str(emp.get("DENOM_CIA", "")).strip()
            tickers_str = str(emp.get("TICKERS", "")).strip()
            ...
            tickers_list = [t.strip() for t in tickers_str.split(",") if t.strip()] if tickers_str else []
            primary_ticker = tickers_list[0] if tickers_list else ""
            cnpj_clean = cnpj.replace(".", "").replace("/", "").replace("-", "")
            setor = ticker_mapper.get_setor(cnpj_clean)
            ...
            errors.append({"cd_cvm": str(emp.get("CD_CVM", "?")), "error": str(e)})

    logger.info(f"Screener: {len(results)} empresas com indicadores, {len(errors)} erros")

    # ── Fetch batch market data for valuation (only for companies with tickers) ──
    tickers_with_results = [r["ticker"] for r in results if r["ticker"]]
    logger.info(f"Screener: buscando market data para {len(tickers_with_results)} tickers (de {len(results)} total)")
    try:
    ...
    for row in results:
        ...
        # Daily market data (for home page)
        row["_change_pct"] = md.get("change_pct")
        row["_change"] = md.get("change")
        row["_volume"] = md.get("volume")
        row["_logo"] = md.get("logo")
        if md.get("name"):
            row["nome_mercado"] = md.get("name")

    # Collect all unique metric keys (exclude internal _ fields)
    all_keys = set()
    for r in results:
        for k in r:
            if k not in ("ticker", "tickers", "nome", "nome_mercado", "setor", "cd_cvm", "DT_REFER") and not k.startswith("_"):
                all_keys.add(k)
...
async def btc_endpoint(
    ...
    """
    Dados de empréstimo de ativos (BTC) da B3.
    Sem ticker: retorna top 50 por valor em aberto.
    Com ticker: retorna posição específica.
    Fonte: Boletim Diário B3 — Posição em Aberto.
    """
    btc_data = fetch_btc_data()

    if ticker:
        result = get_btc_for_ticker(ticker.upper(), btc_data)
        return result

    # Sem ticker: retorna ranking
...
@app.get("/btc/debug")
async def btc_debug():
    """Debug: testa download do PDF da B3 e retorna diagnóstico."""
    return debug_btc_fetch()
