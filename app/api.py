...
async def get_overview(
    ...
    # Resolve ticker se necessário
    resolved_ticker = ticker
    if not resolved_ticker and (cnpj or cd_cvm):
        # Tenta encontrar o ticker pela busca
        search_q = cnpj or cd_cvm
        search_df = data_service.search(search_q)
        if search_df is not None and not search_df.empty:
            first = search_df.iloc[0]
            resolved_ticker = first.get("ticker")
