...
async def get_indicadores(
    ...
    # Fetch market data for valuation multiples
    price_history = None
    shares = None
    resolved_ticker = ticker
    if include_market and resolved_ticker:
        try:
            ...
            shares = market.get("valuation", {}).get("shares_outstanding")
            logger.info(f"Indicadores: {len(price_history or [])} preços, shares={shares}")
        except Exception as e:
