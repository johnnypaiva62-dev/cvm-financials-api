...
def get_combined_overview(cvm_overview: dict, market_data: dict) -> dict:
    ...
    if ev and receita and receita != 0:
        ...
        if ebit and ebit != 0:
            ...
            combined["valuation"]["ev_ebit"] = _safe_round(ev / ebit)
        # EV/EBITDA recalculado com EBITDA CVM
        ebitda = margins.get("ebitda_value")
        if ebitda and ebitda != 0:
            calc["ev_ebitda_cvm"] = _safe_round(ev / ebitda)
            combined["valuation"]["ev_ebitda"] = _safe_round(ev / ebitda)

    fcf = cf.get("fcf")
    if market_cap and fcf and fcf != 0:
        calc["price_to_fcf"] = _safe_round(market_cap / fcf)

    # Leverage com dados de mercado (EV-based)
    lev = cvm_overview.get("leverage", {})
    if ev and lev.get("ebitda") and lev["ebitda"] > 0:
        lev["ev_ebitda"] = _safe_round(ev / lev["ebitda"])
...
def fetch_long_term_prices(ticker: str) -> tuple[list[dict], float | None]:
    """
    Busca histórico MENSAL de preços com range máximo via Yahoo Finance.
    Retorna (price_history, shares_outstanding).
    Usado pelo endpoint /indicadores para valuation histórico completo.
    """
    import yfinance as yf

    suffix = ".SA" if not ticker.upper().endswith(".SA") else ""
    ...
    try:
        ...
        logger.info(f"Long-term prices for {ticker}: {len(records)} monthly points, shares={shares}")
        return records, shares

    except Exception as e:
        logger.warning(f"fetch_long_term_prices falhou para {ticker}: {e}")
        return [], None


def fetch_batch_market_data(tickers: list[str]) -> dict[str, dict]:
    """
    Busca preço atual + market cap + EV + variação diária para múltiplos tickers via BRAPI batch.
    BRAPI suporta até ~20 tickers por request.
    
    Returns:
        Dict[ticker] = {price, shares, market_cap, enterprise_value, change_pct, change, volume, name, logo}
    """
    import requests as req

    if not tickers or not BRAPI_TOKEN:
        logger.warning("fetch_batch_market_data: sem tickers ou BRAPI_TOKEN")
        return {}

    result = {}
    batch_size = 20  # BRAPI limit per request
    ...
    for i in range(0, len(tickers), batch_size):
        ...
        try:
            ...
            for item in data.get("results", []):
                ...
                price = _safe(item.get("regularMarketPrice"))
                mc = _safe(item.get("marketCap"))
                shares = None
                ev = None

                # Try to get shares from defaultKeyStatistics
                ks = item.get("defaultKeyStatistics", {})
                if ks:
                    shares = _safe(ks.get("sharesOutstanding"))
                    ev = _safe(ks.get("enterpriseValue"))
                ...
                if price:
                    result[sym] = {
                        ...
                        "market_cap": mc,
                        "enterprise_value": ev,
                        "change_pct": _safe_round(item.get("regularMarketChangePercent")),
                        "change": _safe_round(item.get("regularMarketChange")),
                        "volume": _safe(item.get("regularMarketVolume")),
                        "name": _safe(item.get("longName") or item.get("shortName")),
                        "logo": _safe(item.get("logourl")),
                    }

            logger.info(f"BRAPI batch {i//batch_size+1}: {len(batch)} tickers, {len([s for s in batch if s.upper() in result])} com dados")

        except Exception as e:
            logger.warning(f"BRAPI batch falhou (offset {i}): {e}")

    logger.info(f"Batch market data: {len(result)}/{len(tickers)} tickers com dados")
    return result
