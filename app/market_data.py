...
def get_market_data(ticker: str) -> dict:
    """
    Busca dados de mercado: tenta BRAPI primeiro, fallback para Yahoo.
    ...
    if brapi_raw:
        logger.info(f"✓ Usando dados BRAPI para {ticker}")
        return _parse_brapi(brapi_raw, ticker)

    # Fallback Yahoo
    logger.info(f"⟳ Fallback Yahoo Finance para {ticker}")
