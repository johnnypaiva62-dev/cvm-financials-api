...
def compute_overview(dre_df, bpa_df, bpp_df, dfc_df, dre_annual_df=None):
    ...
    # ── FLUXO DE CAIXA + EBITDA (precisa vir antes de financial_health e leverage) ──
    ...
    try:
        _compute_history(result, annual, bpa_df, bpp_df, dfc_df)
    except Exception as e:
        import traceback
        logger.warning(f"Erro ao computar histórico: {e}\n{traceback.format_exc()}")
        result["history"] = {
            "receita": [], "lucro": [], "ebit": [], "ebitda": [],
            "margem_liquida": [], "margem_bruta": [], "margem_ebit": [],
            "margem_ebitda": [], "margem_fcf": [],
            "roa": [], "roe": [], "roic": [],
            "dl_ebitda": [], "dl_ebit": [], "dl_pl": [], "db_pl": [], "ebit_juros": [],
            "fco": [], "fcf": [], "capex": [], "dividendos": [],
        }

    return result


def _compute_history(result, annual, bpa_df, bpp_df, dfc_df):
    """Computa séries históricas anuais para gráficos."""
...
def compute_indicadores(dre_df, bpa_df, bpp_df, dfc_df, price_history=None, shares_outstanding=None):
    """
    Computa indicadores financeiros por período (trimestral ou anual).
    Junta DRE + BPA + BPP + DFC por DT_REFER e calcula métricas derivadas.
    
    Args:
        price_history: list of {"dt": "2024-12-31", "close": 58.20} (optional)
        shares_outstanding: float (optional) — shares count for market cap
    
    Returns lista de dicts ordenada por data.
    """
    if dre_df is None or dre_df.empty:
        return []

    # Build price lookup: for each date, find closest price
    price_map = {}
    if price_history and shares_outstanding:
        from datetime import datetime, timedelta
        sorted_prices = sorted(price_history, key=lambda x: x.get("dt", ""))
        price_dates = []
        for p in sorted_prices:
            try:
                price_dates.append((datetime.strptime(p["dt"][:10], "%Y-%m-%d"), p.get("close")))
            except:
                continue

        def _find_price(dt_str):
            """Find closest price to a given date (within 30 days)."""
            try:
                target = datetime.strptime(dt_str[:10], "%Y-%m-%d")
            except:
                return None
            best = None
            best_dist = timedelta(days=31)
            for pd_dt, px in price_dates:
                dist = abs(pd_dt - target)
                if dist < best_dist:
                    best_dist = dist
                    best = px
            return best if best_dist <= timedelta(days=30) else None

    # Indexa BPA, BPP, DFC por DT_REFER
    def _build_map(df):
        m = {}
        if df is None or df.empty:
            return m
        for _, row in df.iterrows():
            dt = _get(row.to_dict(), "DT_REFER")
            if dt:
                m[str(dt)[:10]] = row.to_dict()
        return m

    bpa_map = _build_map(bpa_df)
    bpp_map = _build_map(bpp_df)
    dfc_map = _build_map(dfc_df)
    ...
    for _, row in dre_df.iterrows():
        ...
        # BPA/BPP
        bpa = bpa_map.get(dt_str, {})
        bpp = bpp_map.get(dt_str, {})
        dfc = dfc_map.get(dt_str, {})

        at = _get(bpa, "Ativo Total")
        cx = (_get(bpa, "Caixa e Equivalentes de Caixa", 0) or 0) + (_get(bpa, "Aplicações Financeiras", 0) or 0)
        pl = _get(bpp, "Patrimônio Líquido Consolidado")
        emp_cp = _get(bpp, "Empréstimos e Financiamentos CP", 0) or 0
        emp_lp = _get(bpp, "Empréstimos e Financiamentos LP", 0) or 0
        db = emp_cp + emp_lp
        dl = db - cx

        # DFC
        fco = _get(dfc, "Caixa Líquido Atividades Operacionais")
        capex_i = (_get(dfc, "Aquisição de Imobilizado", 0) or 0) + (_get(dfc, "Aquisição de Intangível", 0) or 0)
        fcf = (fco or 0) + capex_i if fco else None
        div_pg = _get(dfc, "Pagamento de Dividendos")
        da = _get(dfc, "Depreciação e Amortização", 0)
        ebitda = (eb or 0) + abs(da or 0) if eb is not None else None

        # ROIC
        roic = None
        if eb and pl and dl is not None:
            nopat = eb * (1 - BRAZIL_TAX_RATE)
            ci = (pl or 0) + max(dl, 0)
            if ci > 0:
                roic = round((nopat / ci) * 100, 2)

        period = {
            "DT_REFER": dt_str,
            # Resultados
            "Receita Líquida": _safe_round(rec, 0),
            "Resultado Bruto": _safe_round(res_bruto, 0),
            "EBITDA": _safe_round(ebitda, 0),
            "EBIT": _safe_round(eb, 0),
            "Lucro Líquido": _safe_round(luc, 0),
            # Margens (%)
            "Margem Bruta": _safe_pct(res_bruto, rec),
            "Margem EBITDA": _safe_pct(ebitda, rec),
            "Margem EBIT": _safe_pct(eb, rec),
            "Margem Líquida": _safe_pct(luc, rec),
            "Margem FCF": _safe_pct(fcf, rec),
            # Retornos (%)
            "ROE": _safe_pct(luc, pl),
            "ROA": _safe_pct(luc, at),
            "ROIC": roic,
            # Alavancagem
            "Dívida Líquida": _safe_round(dl, 0),
            "Dívida Bruta": _safe_round(db, 0),
            "Patrimônio Líquido": _safe_round(pl, 0),
            "Dív.Líq/EBITDA": _safe_round(_safe_div(dl, ebitda), 2) if ebitda and ebitda > 0 else None,
            "Dív.Líq/EBIT": _safe_round(_safe_div(dl, eb), 2) if eb and eb != 0 else None,
            "Dív.Líq/PL": _safe_round(_safe_div(dl, pl), 2),
            "Dív.Bruta/PL": _safe_round(_safe_div(db, pl), 2),
            "EBIT/Desp.Fin": _safe_round(_safe_div(eb, desp_fin), 2) if desp_fin else None,
            "EBITDA/Desp.Fin": _safe_round(_safe_div(ebitda, desp_fin), 2) if desp_fin and ebitda else None,
            # Fluxo de Caixa
            "FCO": _safe_round(fco, 0),
            "CAPEX": _safe_round(capex_i, 0),
            "FCF": _safe_round(fcf, 0),
            "Dividendos Pagos": _safe_round(div_pg, 0),
        }

        # Valuation (requires price + shares)
        if price_history and shares_outstanding:
            px = _find_price(dt_str)
            if px and px > 0:
                mc = px * shares_outstanding
                ev = mc + dl
                period["Preço"] = _safe_round(px, 2)
                period["Market Cap"] = _safe_round(mc, 0)
                period["EV"] = _safe_round(ev, 0)
                period["P/E"] = _safe_round(_safe_div(mc, luc), 1) if luc and luc > 0 else None
                period["P/B"] = _safe_round(_safe_div(mc, pl), 1) if pl and pl > 0 else None
                period["EV/EBITDA"] = _safe_round(_safe_div(ev, ebitda), 1) if ebitda and ebitda > 0 else None
                period["EV/EBIT"] = _safe_round(_safe_div(ev, eb), 1) if eb and eb > 0 else None
                period["EV/Sales"] = _safe_round(_safe_div(ev, rec), 1) if rec and rec > 0 else None
                period["P/Sales"] = _safe_round(_safe_div(mc, rec), 1) if rec and rec > 0 else None
                period["P/FCF"] = _safe_round(_safe_div(mc, fcf), 1) if fcf and fcf > 0 else None

        results.append(period)

    results.sort(key=lambda x: x["DT_REFER"])
    return results
