...
# ============================================================
# Ajuste conforme necessidade. Cada ano = ~1 download de ~50-100MB
DEFAULT_ITR_YEARS = [2022, 2023, 2024, 2025]
DEFAULT_DFP_YEARS = [2022, 2023, 2024]
...
class DataService:
    ...
    def get_statement(
        ...
        if cnpj:
            ...
            df = df[
                df["CNPJ_CIA"].str.replace(r"[./-]", "", regex=True) == cnpj_clean
            ]

        if cd_cvm:
            df = df[df["CD_CVM"].astype(str) == str(cd_cvm)]
